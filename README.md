...
PhishGuard-X is a hybrid phishing detection system developed in Python that combines Machine Learning and Rule-Based Analysis to identify phishing URLs and malicious websites. The application provides an interactive Streamlit dashboard for URL and QR code analysis, redirect tracking, campaign detection, and forensic PDF report generation.
...
- Generate forensic reports for phishing investigations.
...
A sample forensic PDF report generated by **PhishGuard-X** is available in the `reports/` directory.
